...
def box_is_room_or_door(original_room_types, box_number,box_count):
    num_doors = 0
    for room in original_room_types:
        if room > 10:
            num_doors += 1
    if box_count < box_count-num_doors:
        return "room"
    else:
        return "door"


def associate_edges_with_rooms(edges, boxes, room_or_door,original_room_types):
    set_boxes=set()
    new_boxes = []
    ...
    for edge in edges:
        for i, box in enumerate(boxes):
            if is_edge_inside_box(edge, box):
                set_boxes.add(i)
    for i_set in set_boxes:
        new_boxes.append(boxes[i_set])
    return new_boxes
