...
import backend.canvas_interactions as canvas_interactions
from globals import ROOM_TYPES, ROOM_COLORS

from PIL import Image, ImageTk
from assets.photos import photos

from frontend.styles import PinkPallete as P
...
class CreateEditCanvasFrame(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        Button(text="Add Room", master = self,command=lambda: canvas_interactions.draw_random_room())\
            .grid(row=0, column=0, sticky='w')
        Button(text="Add Horizontal Door", master=self, command=lambda: canvas_interactions.draw_random_door("horizontal"))\
            .grid(row=0, column=1, sticky='w')
        Button(text="Add Vertical Door", master=self, command=lambda: canvas_interactions.draw_random_door("vertical"))\
            .grid(row=0, column=2, sticky='w')
        
        Combobox(master=self, textvariable=canvas_interactions.room_type_sv, state= "readonly",
                  values = ["Living Room",
                            "Kitchen",
                            "Bedroom",
                            "Bathroom",
        ...
        Entry(master = self, width=10, textvariable=canvas_interactions.neigh_room_indexes_sv)\
            .grid(row=2, column= 1, sticky='w')
        
        Label(text="Neighbor Room Types:", master=self, )\
            .grid(row=3, column=0, sticky="w")
        Entry(master = self, width=10, textvariable=canvas_interactions.neigh_room_types_sv)\
            .grid(row=3, column= 1, sticky='w')
        
        Label(text="Neighbor Door Indexes:", master=self, )\
            .grid(row=4, column=0, sticky="w")
        Entry(master = self, width=10, textvariable=canvas_interactions.neigh_door_indexes_sv)\
            .grid(row=4, column= 1, sticky='w')
        
        Label(text="Neighbor Door Types:", master=self)\
            .grid(row=5, column=0, sticky="w")
        Entry(master = self, width=10, textvariable=canvas_interactions.neigh_door_types_sv)\
            .grid(row=5, column= 1, sticky='w')
        
        Label(text="Selected index: ", master=self)\
            .grid(row=6, column=0)
        Label(textvariable=canvas_interactions.selected_index_sv, master=self)\
            .grid(row=6, column=1, sticky='w')
        
        Label(text="Selected type: ", master=self)\
            .grid(row=7, column=0)
        Label(textvariable=canvas_interactions.selected_type_sv, master=self)\
...
class CreateCanvasOptionsFrame(ttk.Frame):
    def __init__(self, parent,file_path_name,reorganized_json, edit_json_frame, main_frame, notebook_plots:ManagerScrollableNotebook):
        ...
        Button(self, text="Save new Json",
                command=lambda: canvas_interactions.on_close(file_path_name, main_frame.message_label_middle))\
                    .grid(row=next(row_i), column=2, sticky="nw")
        Button(self, text="Clear", command=lambda: canvas_interactions.on_clear(edit_json_frame))\
                    .grid(row=next(row_i), column=2, sticky="nw")
        Button(self, text="Room Selection", command=lambda: canvas_interactions.draw_boxes(reorganized_json))\
                    .grid(row=next(row_i),column=2, sticky='nw')
        Button(self, text="Edge selection", command=lambda: canvas_interactions.draw_edges(reorganized_json), state=DISABLED)\
                    .grid(row=next(row_i), column=2, sticky="nw")
        Button(self, text="Both Selection", command=lambda: canvas_interactions.move_edges_and_boxes_together())\
                    .grid(row=next(row_i), column=2, sticky='nw')
        Button(self,  text="Generate with GCP", command=lambda: \
               canvas_interactions.generate_floorplan(self, file_path_name, main_frame.message_label_middle,notebook_plots))\
                    .grid(row=next(row_i), column=2, sticky='nw')
        Button(self,  text="Generate with local .pth", command=lambda: \
            canvas_interactions.generate_floorplan_local_model(self, file_path_name, main_frame.message_label_middle,notebook_plots))\
                .grid(row=next(row_i), column=2, sticky='nw')
        
        Button(self,  text="Print tags", command=lambda: \
               canvas_interactions.print_all_tags())\
                    .grid(row=next(row_i), column=2, sticky='nw')
