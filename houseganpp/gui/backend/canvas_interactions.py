...
from frontend.draw import draw_rectangle_and_label, draw_line
from local_model.houseganapp_min.model_main import generate
...
def init_gui(edit_json_frame, width, height, _reorganized_json, command:str):
    ...
    global canvas, combobox, MAX_X, MAX_Y, simplified_json, selected_edge
    ...
    MAX_X = width-500
    MAX_Y = height-100
    simplified_json = _reorganized_json
    ...
    if command != "clear":
        ...
        draw_boxes(simplified_json)
        draw_edges(simplified_json)

def draw_boxes(simplified_json):
    ...
    if room_map != {}: # rooms have not been drawn yet
        ...
        for room in {**simplified_json["rooms"],**simplified_json["doors"]}.items():
            room_index = room[0]
...
def draw_random_box(random_box, room_or_door_type, room_index):
        global canvas, simplified_json, room_map
        ...
        simplified_json["room_types"].insert(room_index, room_type)
...
def draw_random_edge(random_edges,box_index, rooms_or_doors, together_room_index, door_type=None):
    global canvas, simplified_json, edge_map
    for edge in random_edges:
        ...
        edge_map[item_id] = edge
        try:
            simplified_json[rooms_or_doors][box_index]["edges"].append(edge)
        except Exception as e:
            traceback.format_exc(e)
...
def draw_random_door(direction):
    ...
    new_door_index = len(simplified_json["rooms"])+len(simplified_json["doors"])
    try:
        simplified_json["doors"].update({new_door_index:{"boxes":random_door, "edges":[], "ed_rm":[], 
                                                      "room_type":room_type_sv.get()}})
    except KeyError as e:
        if str(e) == new_door_index:
            simplified_json["doors"][new_door_index] = {"boxes":random_door,"edges":[],"ed_rm":[],
                                                     "room_type":room_type_sv.get()}
    ...
    for item in restoappend:
        try:
            simplified_json["doors"][new_door_index]["ed_rm"].append(item)
        except KeyError as e:
            simplified_json["doors"][new_door_index]["ed_rm"] = [item]
...
def draw_random_room():
    ...
    try:
        simplified_json["rooms"].update({room_index:{"boxes":random_box, "edges":[], "ed_rm":[], 
                                                      "room_type":room_type}})
    except KeyError as e:
        if str(e) == room_index:
            simplified_json["rooms"][room_index] = {"boxes":random_box,"edges":[],"ed_rm":[],
                                                     "room_type":room_type}
    ...
    for item in restoappend:
        try:
            simplified_json["rooms"][room_index]["ed_rm"].append(item)
        except KeyError as e:
            simplified_json["rooms"][room_index]["ed_rm"] = [item]

def update_new_coords():
    global current_rectangle, action_type, room_map, canvas, room_index_together
    global room_edge_selection, simplified_json
    ...
    if current_rectangle:
        ...
        if current_rectangle in room_map:
            ...
            simplified_json[rooms_or_doors][room_index]["boxes"] = new_coords
        ...
        if room_edge_selection:
            ...
            for i, edge in enumerate(update_edges_coords):
                ...
                simplified_json[rooms_or_doors][room_index_together]["edges"][i][:4] = new_coords
...
def on_close(path, message_label_middle):
    ...
    """
    global simplified_json

    if len(simplified_json["room_types"])!=0:
        if not edge_map:
            draw_edges(simplified_json)
        if not room_map:
            draw_boxes(simplified_json)
        ...
        # reorganized_json = edit_json.get_simplified_format_json(reorganized_json)
        original_format_json = edit_json.get_original_format_json(simplified_json)
        new_path = edit_json.save_json(path, original_format_json)
...
def on_clear(edit_json_frame, system_clear=False):
    global edge_map, room_map, simplified_json, edge_selection, room_selection, ed_rm_list
    ...
    edge_map={}
    room_map = {}
    simplified_json={"room_types":[], "rooms":{}, "doors":{}}
    ed_rm_list = []
    ...
    if not system_clear:
        init_gui(edit_json_frame, 800, 600, simplified_json, "clear")

def get_last_room_index():
    if len(list(simplified_json["rooms"])) == 0:
        room_index = 0
    else:
        room_index = list(simplified_json["rooms"])[-1]+1
    return room_index
...
def increment_doors_index(new_room_index, door_indexes):
    ...
    combined_ed_list = []
    for value in {**simplified_json["rooms"], **simplified_json["doors"]}.values():
    ...
    for key,val in simplified_json["doors"].items():
        new_doors_dict[key+1] = val

    del simplified_json["doors"]
    simplified_json["doors"] = new_doors_dict
...
def update_edges_and_ed_rm_to_add_random_room(room_index, room_type, room_types, room_indexes, door_indexes, door_types):
    ...
    for i, index in enumerate(room_indexes):
        ...
        dir=find_dir(i)
        simplified_json["rooms"][index]["ed_rm"][dir] = [index, room_index]
        simplified_json["rooms"][index]["edges"][dir][5] = room_type 
    ...
    for i, index in enumerate(door_indexes):
        ...
        dir=find_dir(i)
        simplified_json["doors"][index]["ed_rm"][dir] = [index, room_index]
        simplified_json["doors"][index]["edges"][dir][5] = room_type 
...
def end_drag(event):
    ...
    if current_edge:
        ...
        if current_edge in edge_map:
            ...
            simplified_json["rooms"][room_index]["edges"][edge_index][:4] = new_coords
...
def create_new_tab(notebook_plots, name="Floor Plan"):
    from frontend.frames import WhiteBGFrame
...
def preprocess_generation(path):
    ...
    original_format_json = edit_json.get_original_format_json(simplified_json)
    saved_path = edit_json.save_json(path, original_format_json)
